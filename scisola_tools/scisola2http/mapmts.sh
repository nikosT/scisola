...
do
  ...
  temptext+=`echo "    </streams>"` # >> $xml_path
  temptext+=`echo "  <no_stations>$station_num</no_stations>"` # >> $xml_path
  temptext+=`echo "  <quality>$quality</quality>"` # >> $xml_path
  temptext+=`echo "</moment_tensor>"` # >> $xml_path

  if [ ! -e "$plots_path/$beachball32_${array[1]}.png" ]; then
...
temptext+=`echo "</moment_tensors>"` # >> $xml_path

echo $temptext > $xml_path
